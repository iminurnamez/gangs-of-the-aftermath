...
class PlayerAttackScreen(tools._State):
    def __init__(self):
        super(PlayerAttackScreen, self).__init__()
    ...
    def reconcile_deaths(self, att_dead, def_dead):
        ...
        if len(self.attackers) < 1:
            ...
            task2 = Task(self.new_round, 1500)
    ...
    def make_buttons(self):
        ...
        info = [("attack", self.conduct_battle),
                   ("retreat", self.retreat)]
        left = self.window.centerx - 64
        top = self.window.top + 500
    ...
    def attacker_win(self):
        ...
        try:
            ...
            pass
        self.player.gang.hoods.append(self.neighborhood)
        self.neighborhood.owner = self.player.gang
        self.done = True
        self.next = "MESSAGE_SCREEN"
        texts = ["Your gang", "now controls", "{}!".format(self.neighborhood.name)]
        labels = []
        for text in texts:
    ...
    def defender_win(self):
        ...
        self.done= True
        self.next = "MESSAGE_SCREEN"
        texts = ["Your attack", "was", "unsuccessful"]
        labels = []
        for text in texts:
            labels.append(Label(text, {"midtop": (0, 0)}, text_color="gray20", font_size=48))
        self.persist["messages"] = labels
